...
namespace radioMesh {
    ...
    export let lastPacket: RadioMeshPacket;
    ...
    function handleDataReceived() {
        ...
        while (buffer) {
            lastPacket = RadioMeshPacket.getPacket(buffer);
    ...
    /**
     * Registers code to run when the radio mesh receives a number.
     */
    //% help=radioMesh/on-received-number
    //% blockId=radio_mesh_on_number_drag block="on radio mesh received" blockGap=16
    //% useLoc="radioMesh.onDataPacketReceived" draggableParameters=reporter
    //% group="Receive"
    //% weight=20
    ...
    /**
     * Registers code to run when the radio mesh receives a key value pair.
     */
    //% help=radioMesh/on-received-value
    //% blockId=radio_mesh_on_value_drag block="on radio mesh received" blockGap=16
    //% useLoc="radioMesh.onDataPacketReceived" draggableParameters=reporter
    //% group="Receive"
    //% weight=19
    ...
    /**
     * Registers code to run when the radio mesh receives a string.
     */
    //% help=radioMesh/on-received-string
    //% blockId=radio_mesh_on_string_drag block="on radio mesh received" blockGap=16
    //% useLoc="radioMesh.onDataPacketReceived" draggableParameters=reporter
    //% group="Receive"
    //% weight=18
    ...
    /**
     * Registers code to run when the radio mesh receives a buffer.
     */
    //% help=radioMesh/on-received-buffer blockHidden=1
    //% blockId=radio_mesh_on_buffer_drag block="on radio mesh received" blockGap=16
    //% useLoc="radioMesh.onDataPacketReceived" draggableParameters=reporter
    ...
    /**
     * Returns properties of the last radio mesh packet received.
     * @param type the type of property to retrieve from the last packet
     */
    //% help=radioMesh/received-packet
    //% blockGap=8
    //% blockId=radio_mesh_received_packet block="received packet %type=radio_mesh_packet_property" blockGap=16
    //% group="Receive"
    //% weight=16
    export function receivedPacket(type: number) {
        if (lastPacket) {
            switch (type) {
                case RadioMeshPacketProperty.Time: return lastPacket.time;
                case RadioMeshPacketProperty.SerialNumber: return lastPacket.serial;
                case RadioMeshPacketProperty.SignalStrength: return lastPacket.signal;
            }
        }
        return 0;
    }
     ...
     */
    //% blockId=radio_mesh_packet_property block="%note"
    //% shim=TD_ID blockHidden=1
    export function _packetProperty(type: RadioMeshPacketProperty): number {
        return type;
    }

    export class RadioMeshPacket {
        public static getPacket(data: Buffer) {
            if (!data) return undefined;
            // last 4 bytes is RSSi
            return new RadioMeshPacket(data);
        }

        public static mkPacket(packetType: number) {
            const res = new RadioMeshPacket();
            res.data[0] = packetType;
            return res;
        }

        private constructor(public readonly data?: Buffer) {
            if (!data) this.data = control.createBuffer(RADIO_MESH_MAX_PACKET_SIZE + 4);
        }
    ...
    /**
     * Broadcasts a number over radio mesh to any connected micro:bit in the group.
     */
    //% help=radioMesh/send-number
    //% weight=60
    //% blockId=radio_mesh_datagram_send block="radio mesh send number %value" blockGap=8
    //% group="Send"
    export function sendNumber(value: number) {
        let packet: RadioMeshPacket;

        if (value === (value | 0)) {
            packet = RadioMeshPacket.mkPacket(PACKET_TYPE_NUMBER);
        }
        else {
            packet = RadioMeshPacket.mkPacket(PACKET_TYPE_DOUBLE);
        }
    ...
    */
    //% help=radioMesh/send-value
    //% weight=59
    //% blockId=radio_mesh_datagram_send_value block="radio mesh send|value %name|= %value" blockGap=8
    //% group="Send"
    export function sendValue(name: string, value: number) {
        let packet: RadioMeshPacket;

        if (value === (value | 0)) {
            packet = RadioMeshPacket.mkPacket(PACKET_TYPE_VALUE);
        }
        else {
            packet = RadioMeshPacket.mkPacket(PACKET_TYPE_DOUBLE_VALUE);
        }
     ...
     */
    //% help=radioMesh/send-string
    //% weight=58
    //% blockId=radio_mesh_datagram_send_string block="radio mesh send string %msg"
    //% msg.shadowOptions.toString=true
    //% group="Send"
    export function sendString(value: string) {
        const packet = RadioMeshPacket.mkPacket(PACKET_TYPE_STRING);
     ...
     * and running time to any connected micro:bit in the group.
     */
    //% help=radioMesh/send-buffer
    //% weight=57
    //% advanced=true
    export function sendBuffer(msg: Buffer) {
        const packet = RadioMeshPacket.mkPacket(PACKET_TYPE_BUFFER);
        packet.bufferPayload = msg;
        sendPacket(packet);
    }

    /**
    * Set the radio mesh to transmit the serial number in each message.
    * @param transmit value indicating if the serial number is transmitted, eg: true
    */
    //% help=radioMesh/set-transmit-serial-number
    //% weight=8 blockGap=8
    //% blockId=radio_mesh_set_transmit_serial_number block="radio mesh set transmit serial number %transmit"
    //% advanced=true
    export function setTransmitSerialNumber(transmit: boolean) {
        transmittingSerial = transmit;
    }

    function sendPacket(packet: RadioMeshPacket) {
